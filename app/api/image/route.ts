...
export const PUT = async (req: NextRequest) => {
    ...
    const content = await db.transaction().execute(async trx => {
        const {id: contentId} = await routeHandlers.put.content({session, trx, headers: newHeaders, req});
        return await routeHandlers.get.content({contentId, session, trx, headers: newHeaders, req});
    });
