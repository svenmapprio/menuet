...
const checkEmissionConnection = async () => {
  const socketServerConnected = async () => {
    const res = await axios
      .get("http://localhost:4010/connection")
      .catch((e) => console.log("get connection error"));

    if (res?.data !== true) throw "not connected";
...
const handler = async (
  ...
  try {
    ...
    if (paths.length !== 1) throw new ApiError(ErrorCode.PathNotFound);

    const handler = routeHandlers[method][
      path as keyof (typeof routeHandlers)[typeof method]
    ] as (o: any) => Promise<any>;
    ...
    const session = await getSession(newHeaders).catch((e) =>
    ...
    const data = await db.transaction().execute(async (trx) => {
      ...
      if (typeof guard === "function") await guard(args);
