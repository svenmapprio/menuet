...
const ConversationView: FC<{ conversation: Returns.ConversationDetails }> = ({
  ...
  const handleKeyUp = useCallback<KeyboardEventHandler<HTMLTextAreaElement>>(
    (e) => {
      if (e.key.toLowerCase() === "enter" && !e.shiftKey) {
        e.preventDefault();
        if (!putMessage.isLoading) handleSubmit();
      }
    },
    ...
    <div>
      ...
      <div>
        {messages.map(({ message, user }) => (
          <MessageView key={message.id} message={{ message, user }} />
        ))}
      </div>
