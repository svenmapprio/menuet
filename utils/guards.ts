...
const guardHelpers = {
  ...
  conversation: {
    isParticipant: async (
      ...
      !!(await trx
        ...
        .innerJoin("conversationUser", (join) =>
          join
  ...
  user: {
    isFriend: async (
      ...
      const findRequest = async (a: number, b: number) =>
        trx
          .selectFrom("friend")
          .select("userId")
...
const guards: PublicRouteGuards = {
  delete: {
    userPost: async ({ trx, postId, userId, session }) => {
      if (!(await guardHelpers.post.isOwner(trx, postId, session)))
  ...
  get: {
    ...
    content: async ({ session, contentId, trx }) => {
      if (
        !(await guardHelpers.content.isShared(
          trx,
          contentId,
          session.user.id
        )) &&
    ...
    conversation: async ({ conversationId, session, trx }) => {
      if (
        !(await guardHelpers.conversation.isParticipant(
          trx,
          conversationId,
          session
        ))
