...
export const SessionContextProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const sessionData = useQuery({
    queryFn: async (ctx) => {
      const headers = {
        Authorization: codeRef.current ? `Bearer ${codeRef.current}` : "",
      };

      codeRef.current = null;

      const session = await domains.public.get.session(
        {},
    ...
    if (typeof google !== "undefined" && !googleClient) {
      setGoogleClient(
        google.accounts.oauth2.initCodeClient({
          client_id:
            "40415257648-lln4524kpreapkqkh8lt18lrachk00sa.apps.googleusercontent.com",
  ...
  return (
    <SessionContext.Provider value={{ sessionData, onSignIn, onSignOut }}>
      {children}
